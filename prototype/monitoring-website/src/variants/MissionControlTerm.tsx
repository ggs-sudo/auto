// PROTOTYPE variant E — variant A's three-pane layout wearing variant B's skin:
// the agent-benchmarks terminal palette and monospace type, flat rows instead of cards.
// Same structure as A — runs rail, DAG with drawn edges, session inspector.
...
import "./mission-control-term.css";

export function MissionControlTerm() {
    ...
    <div className="mct">
          ...
          return (
            ...
            >
              <div className="mct-runcard__top">
                <span className={`mct-dot mct-dot--${r.status}`} />
                <span className="mct-runcard__id">{r.run_id.slice(9)}</span>
                {s.openGates > 0 && <span className="mct-badge">{s.openGates}</span>}
              </div>
              <div className="mct-runcard__prompt">{r.prompt}</div>
      ...
      <main className="mct-main">
        <header className="mct-head">
          <div>
            ...
            <p className="mct-head__prompt">{run.manifest.prompt}</p>
          </div>
          <dl className="mct-stats">
            <div>
              <dt>status</dt>
              <dd className={`mct-status mct-status--${run.manifest.status}`}>{run.manifest.status}</dd>
            </div>
            <div>
              <dt>phase</dt>
            ...
            <div>
              <dt>nodes</dt>
              <dd>
                {p.done}/{p.total} <span className="mct-sub">· {p.running} live</span>
              </dd>
            </div>
            <div>
              <dt>spend</dt>
        ...
        {gates.length > 0 && (
          <div className="mct-gatestrip">
            ...
            {gates.map((g) => (
              <button
                key={g.seq}
                className={`mct-gatechip mct-gatechip--${g.kind}`}
      ...
      <aside className="mct-inspect">
        ...
        <div className="mct-inspect__head">
          ...
          <h2>{node.title}</h2>
          <div className="mct-inspect__sub">
          ...
          <>
            <div className="mct-glance">
              <div className="mct-glance__row">
                ...
                {session.monitored && <span className="mct-tag">monitored</span>}
              </div>
            ...
            {layer.map((n) => {
              ...
              return (
                <button
                  ...
                  className={`mct-node mct-node--${n.status} ${selected === key ? "is-on" : ""} ${
                    blocked.has(key) ? "is-blocked" : ""
                  }`}
                  onClick={() => onSelect(key)}
                >
                  <span className={`mct-type mct-type--${n.type}`}>{n.type}</span>
                  <span className="mct-node__title">{n.title}</span>
                  <span className="mct-node__foot">
                    <span>{n.id}</span>
                    {n.resolution_mode === "user" && <span className="mct-tag">user</span>}
                    {n.spawned_graph && n.spawned_graph !== graph.graph_id && (
                      <span className="mct-tag">↳ {n.spawned_graph}</span>
                    )}
                    {ready(n) && <span className="mct-tag mct-tag--ready">ready</span>}
                    {blocked.has(key) && <span className="mct-tag mct-tag--blocked">gated</span>}
                  </span>
                  {gate && <span className="mct-node__gate">needs you · #{gate.seq}</span>}
                </button>
              );
            })}
          </div>
...
function GateAction({
  ...
  return (
    <div className={`mct-gate mct-gate--${gate.kind}`}>
      ...
      {gate.artifact && (
        <a className="mct-gate__artifact" href={gate.artifact} target="_blank" rel="noreferrer">
          open the prototype ↗ <span>{gate.artifact}</span>
        </a>
      )}
      {gate.blocking.length > 0 && (
        <div className="mct-gate__blocking">holding: {gate.blocking.join(", ")}</div>
      )}
      <textarea
        value={text}
        placeholder={
      ...
      />
      <div className="mct-gate__actions">
        {gate.kind === "prototype-review" && (
          <>
            <button className="mct-btn mct-btn--go" onClick={() => send("approve")}>
              Approve
            </button>
            <button className="mct-btn" disabled={!text.trim()} onClick={() => send("revise")}>
              Request revision
            </button>
          </>
        )}
        {gate.kind === "task-completion" && (
          <>
            <button className="mct-btn mct-btn--go" disabled={!text.trim()} onClick={() => send("done")}>
              Done
            </button>
            <button className="mct-btn" onClick={() => send("cannot")}>
              Can't do it
            </button>
          </>
        )}
        {(gate.kind === "escalated-question" || gate.kind === "ping") && (
          <button className="mct-btn mct-btn--go" disabled={!text.trim()} onClick={() => send("answered")}>
            Send answer
          </button>
        )}
      </div>
...
function Conversation({ events }: { events: { t: string; kind: string; tool?: string; text: string }[] }) {
  ...
  return (
    <div className="mct-convo" ref={box}>
      {events.map((e, i) => (
        <div key={i} className={`mct-ev mct-ev--${e.kind}`}>
          <div className="mct-ev__gutter">
            <span className="mct-ev__time">{clock(e.t)}</span>
            <span className="mct-ev__kind">{e.tool ?? e.kind}</span>
          </div>
          <div className="mct-ev__body">{e.text}</div>
        </div>
      ))}
